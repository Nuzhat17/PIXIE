...
def manage():
    ...
    if request.method == "POST":
        ...
        db.execute("DELETE FROM memos WHERE date=:date", date=x.strftime("%x"))
...
def post():
    ...
    if request.method == "POST":
        if not request.form.get("post"):
            ...
            x = datetime.datetime.now()
            db.execute("INSERT INTO memos (id, post, tags, day, date, time, postID) VALUES (:id, :p, :t, :d, :date, :time, :pID)",
                       id=session["user_id"], p=request.form.get("post"), t=request.form.get("tags"), d=x.strftime("%a"), date=x.strftime("%x"), time=x.strftime("%X"), pID=request.form.get("post"))
...
def login():
    ...
    if request.method == "POST":
        if not request.form.get("username"):
            return("You must enter a username", 403)

        elif not request.form.get("password"):
            return("You must enter password", 403)
